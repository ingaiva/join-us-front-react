...
const CreativeCards = () => {
  ...
  return (
    <Fragment>
      <Card className="card card-g-info  w-75">
        <CardBody className='stat-card-body py-1 px-2'>
          <p className='p-stat-value border border-2 rounded-circle'>
            {context.giverActions()}
          </p>

          <p className='p-stat-text'>
            {statutLst.find((st) => st.value === context.actionGiver)?.description}
            {/* Nombre de fois où vous avez proposé un service via votre {process.env.REACT_APP_TITLE_MOBILE_APP} */}
          </p>
        </CardBody>
      </Card>
    </Fragment>
